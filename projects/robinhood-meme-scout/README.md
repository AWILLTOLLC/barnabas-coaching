...
4. Score ≥ 70 → **momentum check**: fetches `token info` for the finalist and rejects it if it's down more than `max_drop_6h_pct` (default 20%) over 6h — catches post-pump dumps that list-payload data can't see (logged as `momentum_reject`).
...
8. **Feedback loop (v2.1):** every coin seen — rejects included — is recorded once in `data/scout.db` (SQLite, `node:sqlite`) with its full snapshot and evaluation; price/liquidity outcomes are captured at +1h/6h/24h/72h/168h (≤5 fetches per tick, 3 failures = marked dead, itself a rug signal). A **daily report DM at 7am PT** compares median returns across bands (alerted vs near-miss vs rejected), names rejects that ran, counts dead coins, and adds an Ollama narrative suggesting which criteria parameter to review. Criteria changes stay human-approved — edit `criteria.json`, restart. Nothing in this repo trades; execution is manual and stays that way. On-demand: `tsx src/cli.ts report --dry-run`.
