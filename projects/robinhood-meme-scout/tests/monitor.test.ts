...
test('scanOnce evaluates, alerts on high scores, dedupes known addresses', async (t) => {
  ...
  const mon = new Monitor({
    ...
    fetchStats: async (address) => { statsCalls.push(address); return { price: 0.001, change_6h_pct: 5, change_24h_pct: 10 }; },
...
test('finalist dumping 6h is momentum-rejected, no alert', async (t) => {
  ...
  const mon = new Monitor({
    ...
    fetchStats: async () => ({ price: 0.001, change_6h_pct: -60, change_24h_pct: -80 }),
